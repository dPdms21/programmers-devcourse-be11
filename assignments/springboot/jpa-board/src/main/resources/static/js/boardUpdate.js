...
let updated = () => {
    $('#submitBtn').on('click', (event) => {
        ...
        let hId = $('#hiddenId').val(); // 수정할 글의 id (URL 경로에 넣음)
        let formData = new FormData($('#writeForm')[0]);

        $.ajax({
            type: 'PUT',
            url: '/api/boards/' + hId, // 수정 대상 글을 경로로 식별 (PUT /api/boards/{id})
            data: formData,
            processData: false,
            contentType: false,
...
let loadBoardDetail = () => {
    ...
    $.ajax({
        ...
        success: (response) => {
            ...
            $('#content').val(response.content); // textarea 값은 .val() 로 채움
            ...
            if (response.filePath && response.filePath.length > 0) {
                ...
                // 경로 구분자를 '/'로 통일한 뒤 마지막 '/' 뒤(파일명)를 자름 (OS별 구분자 차이 대응)
